...
class EncoderRNN(BaseRNN):
    ...
    def __init__(self, feature_size, hidden_size,
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(1, 32, kernel_size=(41, 11), stride=(2, 2), padding=(20, 5)), # conv2d를 사용
        ...
        # 순차적으로 실행하도록 담는 container이다. 


        feature_size = math.ceil((feature_size - 11 + 1 + (5*2)) / 2) # 올림
        feature_size = math.ceil(feature_size - 11 + 1 + (5*2)) # 올림
        feature_size *= 32 #32를 곱함

        self.rnn = self.rnn_cell(feature_size, hidden_size, n_layers,
                                 batch_first=True, bidirectional=bidirectional, dropout=dropout_p)

        # 인스턴스 변수 rnn에 rnn_cell을 집어 넣음

    def forward(self, input_var, input_lengths=None): # 요기는 잠시 보류
        """
        Applies a multi-layer RNN to an input sequence.
        ...
        input_var = input_var.unsqueeze(1) # 새로운 차원을 삽입
